...
/**
 * A CRTP that defines the API for a broker.
 * @tparam Derived A class inheriting from `broker_base`
 */
template <typename Derived> class broker_base : public EWrapper {
public:
   ...
   * @param attribs an `TickAttrib` object that contains price attributes such
   * as `TickAttrib::CanAutoExecute`, `TickAttrib::PastLimit` and
   * `TickAttrib::PreOpen`.
   */
  ...
  void pnl(int reqId, double dailyPnL, double unrealizedPnL,
           double realizedPnL);
  void pnlSingle(int reqId, int pos, double dailyPnL, double unrealizedPnL,
  ...
  void tickByTickAllLast(int reqId, int tickType, time_t time, double price,
                         int size, const TickAttribLast &tickAttribLast,
